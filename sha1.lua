...
local function MainLoop(str)
	...
	while (str ~= "") do
		...
		for i = 0, 79 do
			...
			t = (LeftRotate(a, 5) + f + e + k + w[i])%2^32
			e = d
			d = c
			c = LeftRotate(b, 30)
			b = a
		...
		h0 = band((h0+a)%2^32, 4294967295)
		h1 = band((h1 + b)%2^32, 4294967295)
		h2 = band((h2 + c)%2^32, 4294967295)
		h3 = band((h3 + d)%2^32, 4294967295)
		h4 = band((h4 + e)%2^32, 4294967295)
